...
$page_title = 'Laporan Kondisi Fisik Bahan Pustaka';
$reportView = false;
$num_recs_show = 20;
if (isset($_GET['reportView'])) {
    $reportView = true;
...
?>
    <!-- filter -->
    <div class="per_title">
        <h2><?php echo __('Laporan Kondisi Fisik Bahan Pustaka'); ?></h2>
    </div>
    <div class="infoBox">
    ...
    <div class="sub_section">
        <form method="get" action="<?= $_SERVER['PHP_SELF'] . '?' . httpQuery() ?>" target="reportView">
            ...
            <div id="filterForm">
                ...
                <div class="form-group divRow">
                    <label><?php echo __('Kondisi Bahan Pustaka'); ?></label>
                <?php
                    ...
                    $physicalc_options[] = array('3', __('Rusak Berat'));
                     ...
                     $physicalc = new simbio_fe_radio();
                     $physicalc->element_name= 'physicalCondition';
                     $physicalc->element_helptext="Kondisi Fisik Bahan Pustaka";
                     $physicalc->element_options = array (
                                                            array(1, "Baik"),
                                                            array(2, "Rusak Ringan"),
                                                            array(3, "Rusak Berat")
                                                            );
                     echo $physicalc->out();
                    ...
                    //$form->addRadio('physicalCondition', __('Physical Condition'), $physicalc_options, '1');
...
<?php
    ...
    ob_start();
       ...
       $table_spec = 'item AS i
                   ...
                   LEFT JOIN item_conditions AS icon ON i.item_condition_id=icon.id';
    ...
    $reportgrid->setSQLColumn(
        ...
        'CASE WHEN icon.physical_condition = 0 THEN \''.htmlspecialchars_decode(htmlentities($unchecklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).'\' WHEN icon.physical_condition = 1 THEN \''.htmlspecialchars_decode(htmlentities($checklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).'\' ELSE \''.htmlspecialchars_decode(htmlentities($unchecklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).' \' END AS \'' . __('Baik') . '\'',
        'CASE WHEN icon.physical_condition = 0 THEN \''.htmlspecialchars_decode(htmlentities($unchecklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).'\' WHEN icon.physical_condition = 2 THEN \''.htmlspecialchars_decode(htmlentities($checklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).'\' ELSE \''.htmlspecialchars_decode(htmlentities($unchecklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).' \' END AS \'' . __('Rusak Ringan') . '\'',
        'CASE WHEN icon.physical_condition = 0 THEN \''.htmlspecialchars_decode(htmlentities($unchecklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).'\' WHEN icon.physical_condition = 3 THEN \''.htmlspecialchars_decode(htmlentities($checklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).'\' ELSE \''.htmlspecialchars_decode(htmlentities($unchecklist, ENT_QUOTES|ENT_SUBSTITUTE|ENT_DISALLOWED)).' \' END AS \'' . __('Rusak Berat') . '\'',
        'icon.physical_condition_info AS \'' . __('Keterangan') . '\'',
        'i.biblio_id'
    );
    $reportgrid->setSQLorder('b.title ASC');

    // is there any search
    $criteria = 'b.biblio_id IS NOT NULL AND i.item_condition_id IS NOT NULL AND i.item_condition_id != \'\' ';
    ...
    if(isset($_GET['physicalCondition'])){
           //print_r($_GET['itemMaterial']);
       // foreach ($_GET['physicalCondition'] as $value) {
       $physicalCondition = $_GET['physicalCondition'];
            if($physicalCondition == 1){
              $criteria .= ' AND icon.physical_condition=\'1\'';
              //$criteria .= ' AND imat.property_stamp ='.$value;
            }
            if($physicalCondition == 2){
              $criteria .= ' AND icon.physical_condition = \'2\''; 
               //$criteria .= ' AND imat.inventory_stamp ='.$value;
            }
            if($physicalCondition == 3){
               $criteria .= ' AND icon.physical_condition = \'3\''; 
    ...
    $reportgrid->invisible_fields = array(13);
    ...
    $xlsquery = "SELECT i.item_code AS '" . __('KODE EKSEMPLAR') . "',
            i.inventory_code AS '" . __('KODE INVENTARIS') . "',
            DATE(i.last_update) AS '" . __('TANGGAL SURVEY') . "',
            b.title AS '" . __('JUDUL') . "',
            b.gmd AS '" . __('GMD') . "',
            b.edition AS '" . __('EDISI') . "',
            b.isbn_issn AS '" . __('ISBN/ISSN') . "',
            ...
            CASE WHEN icon.physical_condition = 0 THEN ' ' WHEN icon.physical_condition = 1 THEN 'RUSAK RINGAN' ELSE ' ' END AS '". __('Baik') ."',
            CASE WHEN icon.physical_condition = 0 THEN ' ' WHEN icon.physical_condition = 2 THEN 'RUSAK RINGAN' ELSE ' ' END AS '" . __('Rusak Ringan') ."',
            CASE WHEN icon.physical_condition = 0 THEN ' ' WHEN icon.physical_condition = 3 THEN 'RUSAK BERAT' ELSE ' ' END AS '". __('Rusak Berat')."' FROM " .
